...
public class ContatosDAO {
    ...
    public long create(ContatosBEAN contato) throws SQLException {
    ...
    public void update(ContatosBEAN contato) throws SQLException {
    ...
    public void delete(ContatosBEAN contato) throws SQLException {
    ...
    public ContatosBEAN findContato(int id) {
        ...
        try {
            if (rs.next()) {
                result = new ContatosBEAN(rs.getInt("id"), rs.getString("nome"), rs.getString("apelido"), rs.getDate("data_nascimento"));
            }
            rs.close();
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public int findId(ContatosBEAN contato) {
        int result = 0;
        ResultSet rs = null;
        rs = MySQLDAO.getResultSet("SELECT * FROM Contatos WHERE nome= ? and apelido= ? and data_nascimento = ?", contato.getNome(), contato.getApelido(), contato.getData_nascimento());
        try {
            if (rs.next()) {
                result = rs.getInt("id");
            }
